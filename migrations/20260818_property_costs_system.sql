-- Create property costs table supporting daily or monthly recurring/fixed expenses with invoices
CREATE TABLE IF NOT EXISTS public.property_costs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  property_id BIGINT NOT NULL REFERENCES public.properties(id) ON DELETE CASCADE,
  title VARCHAR(255) NOT NULL,
  cost_type VARCHAR(20) NOT NULL DEFAULT 'monthly', -- 'daily' or 'monthly'
...
DROP POLICY IF EXISTS "Allow public read property costs" ON public.property_costs;
DROP POLICY IF EXISTS "Allow all property costs for admin" ON public.property_costs;
CREATE POLICY "Allow public read property costs" ON public.property_costs FOR SELECT USING (true);
CREATE POLICY "Allow all property costs for admin" ON public.property_costs FOR ALL USING (true);

-- Update landlord_data RPC to include property_costs
...
BEGIN
  ...
  ) ORDER BY c.created_at DESC), '[]'::jsonb) INTO v_costs
  FROM public.property_costs c
  JOIN public.property_landlords pl ON pl.property_id = c.property_id
