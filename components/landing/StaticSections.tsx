  ...
  return (
    <section id="hero" className="relative flex h-screen items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 bg-hs-bg">
        <div className="absolute inset-0 bg-[linear-gradient(135deg,rgba(201,169,110,0.16),rgba(12,10,8,0.25)_34%,rgba(12,10,8,0.95)),radial-gradient(circle_at_70%_20%,rgba(201,169,110,0.28),transparent_30%),linear-gradient(120deg,#211b14,#0c0a08_58%,#17120d)]" />
        <div className="absolute inset-0 bg-gradient-to-b from-hs-bg/30 via-hs-bg/60 to-hs-bg" />
        <div className="absolute inset-0 opacity-20 bg-[radial-gradient(ellipse_at_center,_var(--hs-primary)_0%,_transparent_70%)]" />
        <div className="absolute bottom-0 left-0 right-0 h-44 bg-gradient-to-t from-hs-bg to-transparent" />
      </div>
      {/* @section: hero-content */}
      <div className="relative z-10 mx-auto max-w-5xl px-6 text-center">
        <p className="mb-6 text-sm font-medium uppercase tracking-[0.4em] text-hs-primary">
          {isAr ? 'الرياض · المملكة العربية السعودية' : 'RIYADH · SAUDI ARABIA'}
        </p>
        <h1 className="mb-6 font-serif text-7xl font-bold leading-none tracking-tight text-hs-text md:text-9xl">
          Horizon<br /><span className="text-hs-primary">Stays</span>
        </h1>
        <p className="mb-10 text-xl font-light text-hs-muted md:text-2xl">
          {isAr ? 'وحدات سكنية فاخرة · إقامة استثنائية' : 'Luxury Living · Exceptional Experience'}
        </p>
        <a href="#properties" className="inline-flex items-center gap-3 rounded-full bg-hs-primary px-8 py-4 text-lg font-semibold text-hs-bg transition-all hover:scale-105 hover:bg-hs-primary2">
      ...
      <div className="mx-auto max-w-5xl text-center">
        ...
        <div className="group relative aspect-video overflow-hidden rounded-[2rem] border border-hs-border bg-gradient-to-br from-hs-bg3 via-hs-bg2 to-hs-primary/20 shadow-2xl shadow-black/40">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <button className="flex h-24 w-24 items-center justify-center rounded-full border border-hs-primary/40 bg-hs-bg/70 text-4xl text-hs-primary shadow-xl backdrop-blur transition group-hover:scale-110" aria-label={isAr ? 'تشغيل الجولة' : 'Play tour'}>
              ▶
            </button>
          </div>
          <div className="absolute bottom-6 left-6 right-6 rounded-2xl border border-hs-border bg-hs-bg/70 p-4 text-start backdrop-blur">
            <p className="font-serif text-2xl text-hs-text">{isAr ? 'إطلالة الرياض · تفاصيل فندقية · خصوصية كاملة' : 'Riyadh skyline · Hotel-grade details · Complete privacy'}</p>
          </div>
        </div>
      </div>
    </section>
...
export function FooterSection({ locale }: { locale: string }) {
  const isAr = locale === 'ar';
  return (
    <footer className="border-t border-hs-border bg-hs-bg px-6 py-12">
      {/* @section: footer */}
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-10 md:grid-cols-[1.2fr_1fr_1fr]">
          <div>
            ...
            <p className="mt-4 max-w-sm leading-7 text-hs-muted">
              {isAr ? 'ضيافة فاخرة وإقامات قصيرة المدى مصممة بعناية في قلب الرياض.' : 'Luxury hospitality and carefully curated short-term stays in the heart of Riyadh.'}
            </p>
          </div>
          ...
          <div className="space-y-3 text-hs-muted">
            ...
            <div className="flex gap-3">
              {['Instagram', 'X', 'Snapchat'].map((social) => (
                <a key={social} href="#" className="rounded-full border border-hs-border px-4 py-2 text-sm transition hover:border-hs-primary hover:text-hs-primary">
                  {social}
                </a>
              ))}
            </div>
        ...
        <div className="mt-10 border-t border-hs-border pt-6 text-center text-sm text-hs-muted">
          {isAr ? 'جميع الحقوق محفوظة © 2024 Horizon Stays' : 'All rights reserved © 2024 Horizon Stays'}
        </div>
      </div>
    </footer>
  );
