...
/**
 * Parallax section choreography for the "Artistic Horizon" theme:
 * the hero is pinned and sections slide UP over it while scrolling down,
 * with depth-scaled speeds for a cinematic layered effect.
 */
export function useParallaxSections(enabled: boolean, pathKey?: string) {
  useEffect(() => {
    const root = document.documentElement;
    if (!enabled) {
      root.classList.remove("parallax-mode");
      return;
    }
    const reduced = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (reduced) return;

    root.classList.add("parallax-mode");
    let raf = 0;

    const update = () => {
      const y = window.scrollY;
      const vh = window.innerHeight;
      // Pinned hero: recedes and dims as content slides over it
      const hero = document.querySelector<HTMLElement>(".hero");
      if (hero) {
        const p = Math.min(1, y / vh);
        hero.style.setProperty("--px-scale", String(1 - p * 0.08));
        hero.style.setProperty("--px-dim", String(p * 0.55));
        hero.style.setProperty("--px-shift", `${y * 0.35}px`);
      }
      // Sections after the hero drift up slightly faster than scroll (slide-over feel)
      document.querySelectorAll<HTMLElement>(".px-layer").forEach((el, i) => {
        const r = el.getBoundingClientRect();
        if (r.top < vh && r.bottom > 0) {
          const progress = 1 - r.top / vh; // 0 → entering, 1 → at top
          const lift = Math.min(60, progress * (28 + i * 10));
    ...
    return () => {
      ...
      cancelAnimationFrame(raf);
      root.classList.remove("parallax-mode");
      document.querySelectorAll<HTMLElement>(".px-layer").forEach((el) => (el.style.transform = ""));
    };
  }, [enabled, pathKey]);
}
